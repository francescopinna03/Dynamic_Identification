...
def auction_contraction_certificate(lam: float, chi: float, n_values: int,
                                    n_bids: int, sigma: float) -> dict:
    kernel = signal_kernel(n_values, sigma)
    payoff, _ = auction_payoff(n_values, n_bids)
    n_reduced = n_values * (n_bids - 1)
    basis = np.zeros((n_values * n_bids, n_reduced))
    for signal in range(n_values):
        for bid in range(n_bids - 1):
            column = signal * (n_bids - 1) + bid
            basis[signal * n_bids + bid, column] = 1.0
            basis[signal * n_bids + n_bids - 1, column] = -1.0

    base_policy = np.full((n_values, n_bids), 1.0 / n_bids)
    base_utility = auction_utilities(
        base_policy, chi, kernel, payoff
    ).reshape(-1)
    coefficients = np.column_stack([
        auction_utilities(
            (base_policy.reshape(-1) + basis[:, column]).reshape(
                n_values, n_bids
            ),
            chi,
            kernel,
            payoff,
        ).reshape(-1) - base_utility
        for column in range(n_reduced)
    ]).reshape(n_values, n_bids, n_reduced)

    candidates = []
    for signal in range(n_values):
        for bid in range(n_bids - 1):
            for other_bid in range(n_bids):
                if other_bid == bid:
                    continue
                diameter = float(np.sum(np.abs(
                    coefficients[signal, bid]
                    - coefficients[signal, other_bid]
                )))
                candidates.append((diameter, signal, bid, other_bid))
    diameter, signal, bid, other_bid = max(candidates)
    modulus = float(lam * diameter / 4.0)
    return {
        "reduced_affine_diameter": diameter,
        "contraction_modulus": modulus,
        "certified_unique": bool(modulus < 1.0),
        "critical_lambda": float(4.0 / diameter),
        "maximizing_signal": int(signal),
        "maximizing_bid": int(bid),
        "comparison_bid": int(other_bid),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("design_games.json"))
    ...
    contraction = auction_contraction_certificate(
        eta[0], eta[1],
        final["design"]["G4"]["values"],
        final["design"]["G4"]["bids"],
        final["design"]["G4"]["sigma"],
    )
    stability = finite_difference_stability(eta, final["design"])
    structural = structural_checks(eta, final["design"])
    checks = {
        "optimizer_success": bool(final["success"]),
        "full_rank_battery": bool(final["battery"]["rank"] == 3),
        "unique_auction_equilibrium": bool(
            final["branch_audit"]["distinct_fixed_points"] == 1
        ),
        "auction_contraction": bool(contraction["certified_unique"]),
        "perfect_signal_cursing_collapse": bool(
            structural["G4_perfect_signal_cursing_collapse"]
        ),
        "finite_difference_stability": bool(max(
            row["relative_frobenius_difference_from_1e-5"]
    ...
    payload = {
        ...
        "finite_difference_stability": stability,
        "structural_checks": structural,
        "auction_contraction_certificate": contraction,
        "checks": checks,
    ...
    if not contraction["certified_unique"]:
        raise RuntimeError("G4 contraction certificate failed")
